    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def step(self, action):
        ...
        if self.render_mode == "human":
            self.render()
#             self.scoreRender(self.reward)
        return np.array(self.state, dtype=np.float32), self.reward, terminated, False, {}
...
#         self.screen = pygame.Surface((self.screen_width, self.screen_height))
        ...
        renderReward = self.reward
        score_text = font.render(f'Score: {str(round(self.renderReward)/10)}', True, (0,0,0))
        if self.action == 0:
            push = "Left"
        else:
            push = "Right"
        direction = font.render(push, True, (255,0,0))
        cartwidth = 50.0
        cartheight = 30.0
        ...
        self.screen.blit(direction, (250,10))
        if self.render_mode == "human":
            pygame.event.pump()
            self.clock.tick(self.metadata["render_fps"])
            pygame.display.flip()
            self.renderReward +=self.reward
...
#     def scoreRender(self, renderReward):
#         import pygame
#         pygame.init()
